...
It is recommended to install [Tow-Boot](https://tow-boot.org/) to
dedicated storage (mmcboot).

Mainline U-Boot should work, but Mobile GaoOS is tested with Tow-Boot.
...
```
$ dd if=mobile-gaoos_pine64-pinephone_installer.img of=/dev/mmcblkX bs=8M oflag=sync,direct status=progress
```
...
> **Note**: The following sections only apply for development.
...
GaoOS boot partition.

```
$ nix-build --argstr device pine64-pinephone -A build.boot-partition
$ dd if=result/mobile-gaoos-boot.img of=/dev/mmcblkXpY bs=8M oflag=sync,direct status=progress
```

### Building U-Boot

Mobile GaoOS is not managing platform firmware builds anymore.

We recommend installing [Tow-Boot](https://tow-boot.org/) to dedicated storage
(mmcboot).
