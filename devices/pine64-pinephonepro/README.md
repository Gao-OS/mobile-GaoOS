...
The only **supported** platform firmware configuration is to [install Tow-Boot to SPI](https://tow-boot.org/devices/pine64-pinephonePro.html).
...
```
$ dd if=mobile-gaoos_pine64-pinephonepro_installer.img of=/dev/sdX bs=8M oflag=sync,direct status=progress
```
