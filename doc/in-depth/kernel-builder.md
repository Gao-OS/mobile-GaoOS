...
The "Kernel builder" is a set of attributes available on the `mobile-gaoos`
attribute overlaid on top of Nixpkgs.

```
mobile-gaoos.kernel-builder
mobile-gaoos.kernel-builder-clang
```
...
When true, the build will use `dtbTool` to build a QCDT *device-tree image*.
...
When true, the build will use `dtbTool-exynos` to build a *device-tree image*
for the device.
...
the *logo* to be shown, and centered on the display.
...
By default an appropriate file is provided by Mobile GaoOS.
...
There have been no *weird side-effects* observed when using the wrong value.
